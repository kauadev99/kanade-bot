...
\t\t• Caso esteja usando o *bemvindo* (com foto), use: *${prefix}legendabv sua mensagem..*
\t\t• Usando o *bemvindo2* (sem foto)? Use *${prefix}legendabv2 sua mensagem..*
• Definir uma legenda ou mensagem para a saída de um integrante do grupo:
\t\t• Caso esteja usando o *bemvindo* (com foto), use: *${prefix}legendasaiu sua mensagem..*
\t\t• Usando o *bemvindo2* (sem foto)? Use *${prefix}legendasaiu2 sua mensagem..*`
}

exports.infoOwner = (prefix, NickDono, numerodn, NomeDoBot, sender) => {
return `Bom dia, @${sender.split("@")[0]}, aqui está as\ninformações da minha mestre:
–
• Número: wa.me/${numerodn}
• Nome registrado: ${NickDono}`
}
...
\t• ${prefix}numero-dono 559185731582
